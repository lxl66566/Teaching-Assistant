...
export class KnowledgeAPI {
  ...
  async uploadDocument(file: File, type: string, description?: string) {
    try {
      ...
      const response = await axios.post(
        `${getCurrentBackend().toBase()}/knowledge/upload`,
        formData,
        {
          headers: {
  ...
  async getDocumentStatus(documentId: string): Promise<DocumentStatus> {
    const response = await axios.get(
      `${getCurrentBackend().toBase()}/knowledge/status/${documentId}`,
    );
    return response.data;
  }
  ...
  async getKnowledgeList(params?: {
    ...
    const response = await axios.get(
      `${getCurrentBackend().toBase()}/knowledge/list`,
      { params },
    );
  ...
  async deleteDocument(documentId: string) {
    const response = await axios.delete(
      `${getCurrentBackend().toBase()}/knowledge/${documentId}`,
    );
    return response.data;
  }
